...
ICM_20948_Status_e icm209_clearInterrupts(void)
{
    ...
    retval = ICM_20948_execute_r(&g_driver.internal_dev,
                                 AGB0_REG_INT_STATUS,
                                 (uint8_t*)&int_stat,
                                 sizeof(ICM_20948_INT_STATUS_t));
    if (retval != ICM_20948_Stat_Ok)
    {
        return retval;
    }
    ...
    // todo: there may be additional interrupts that need to be cleared, like
    // FIFO overflow/watermark
...
ICM_20948_Status_e icm209_cfgIntActiveLow(bool active_low)
{
    ...
    ICM_20948_Status_e      retval =
      ICM_20948_int_pin_cfg(&g_driver.internal_dev, NULL, &reg); // read phase
    if (retval != ICM_20948_Stat_Ok)
    {
        return retval;
    }
...
ICM_20948_Status_e icm209_intEnableI2C(bool enable)
{
    ...
    ICM_20948_Status_e     retval =
      ICM_20948_int_enable(&g_driver.internal_dev, NULL, &en); // read phase
    if (retval != ICM_20948_Stat_Ok)
    {
        return retval;
    }
...
ICM_20948_Status_e icm209_setDMPODRrate(enum DMP_ODR_Registers odr_reg, int interval)
{
    ...
    {
        // In order to set an ODR for a given sensor data, write 2-byte value to
        // DMP using key defined above for a particular sensor. Setting value
        // can be calculated as follows: Value = (DMP running rate (225Hz) / ODR
...
ICM_20948_Status_e icm209_initialize_dmp(void)
{
    ...
    {
        LOG_DEBUG("icm209_startupDMP: DMP is not available. Please check that "
                  "you have uncommented line 29 (#define ICM_20948_USE_DMP) in "
                  "ICM_20948_C.h...\n");
        return ICM_20948_Stat_DMPNotSupported;
    }
    ...
    // The ICM-20948 is awake and ready but hasn't been configured. Let's step
    ...
    // magnetometer to run at 100Hz by setting the CNTL2 register (0x31) to
    // 0x08. Then the ICM20948's I2C_SLV0 is configured to read nine bytes from
    // the mag every sample, starting from the STATUS1 register (0x10). ST1
    // includes the DRDY (Data Ready) bit. Next are the six magnetometer
    // readings (little endian). After a dummy byte, the STATUS2 register (0x18)
    // contains the HOFL (Overflow) bit.
    //
    // But looking very closely at the InvenSense example code, we can see in
    // inv_icm20948_resume_akm (in Icm20948AuxCompassAkm.c) that, when the DMP
    // is running, the magnetometer is set to Single Measurement (SM) mode and
    // that ten bytes are read, starting from the reserved RSV2 register (0x03).
    // The datasheet does not define what registers 0x04 to 0x0C contain. There
    // is definitely some secret sauce in here... The magnetometer data appears
    // to be big endian (not little endian like the HX/Y/Z registers) and starts
    // at register 0x04. We had to examine the I2C traffic between the master
    // and the AK09916 on the AUX_DA and AUX_CL pins to discover this...
    //
    // So, we need to set up I2C_SLV0 to do the ten byte reading. The parameters
    // passed to i2cControllerConfigurePeripheral are: 0: use I2C_SLV0
    // MAG_AK09916_I2C_ADDR: the I2C address of the AK09916 magnetometer (0x0C
    // unshifted) AK09916_REG_RSV2: we start reading here (0x03). Secret
    // sauce... 10: we read 10 bytes each cycle true: set the I2C_SLV0_RNW
    // ReadNotWrite bit so we read the 10 bytes (not write them) true: set the
    // I2C_SLV0_CTRL I2C_SLV0_EN bit to enable reading from the peripheral at
    // the sample rate false: clear the I2C_SLV0_CTRL I2C_SLV0_REG_DIS (we want
    // to write the register value) true: set the I2C_SLV0_CTRL I2C_SLV0_GRP bit
    ...
    // 1: use I2C_SLV1
    // MAG_AK09916_I2C_ADDR: the I2C address of the AK09916 magnetometer (0x0C
    // unshifted) AK09916_REG_CNTL2: we start writing here (0x31) 1: not sure
    // why, but the write does not happen if this is set to zero false: clear
    // the I2C_SLV0_RNW ReadNotWrite bit so we write the dataOut byte true: set
    // the I2C_SLV0_CTRL I2C_SLV0_EN bit. Not sure why, but the write does not
    // happen if this is clear false: clear the I2C_SLV0_CTRL I2C_SLV0_REG_DIS
    // (we want to write the register value) false: clear the I2C_SLV0_CTRL
    ...
    // It is not clear why we need to do this... But it appears to be essential!
    // From the datasheet: "I2C_MST_ODR_CONFIG[3:0]: ODR configuration for
    // external sensor when gyroscope and accelerometer are disabled.
    ...
    // Since both gyro and accel are running, setting this register should have
    // no effect. But it does. Maybe because the Gyro and Accel are placed in
    // Low Power Mode (cycled)? You can see by monitoring the Aux I2C pins that
    // the next three lines reduce the bus traffic (magnetometer reads) from
    // 1125Hz to the chosen rate: 68.75Hz in this case.
    result = setBank(&g_driver.internal_dev, 3);
    if (result > worstResult)
    {
    ...
    // ICM_20948_Clock_Auto selects the best available clock source – PLL if
    // ready, else use the Internal oscillator
    result = icm209_set_clock_source(ICM_20948_Clock_Auto);
    if (result > worstResult)
    {
        worstResult = result; // This is shorthand: success will be set to false
                              // if setClockSource fails
    }

    // Enable accel and gyro sensors through PWR_MGMT_2
    // Enable Accelerometer (all axes) and Gyroscope (all axes) by writing zero
    // to PWR_MGMT_2
    result = setBank(&g_driver.internal_dev, 0);
    if (result > worstResult)
    {
    ...
    // Disable the FIFO
    result = icm209_enableFIFO(false);
    if (result > worstResult)
    {
        worstResult = result;
    }
    ...
    uint8_t zero = 0;
    result       = write(&g_driver.internal_dev, AGB0_REG_FIFO_EN_1, &zero, 1);
    if (result > worstResult)
    {
        worstResult = result;
    }
    ...
    result = write(&g_driver.internal_dev, AGB0_REG_FIFO_EN_2, &zero, 1);
    if (result > worstResult)
    {
        worstResult = result;
    }
    ...
    result = icm209_intEnableRawDataReady(false);
    if (result > worstResult)
    {
        worstResult = result;
    }
    ...
    result = setDMPstartAddress(&g_driver.internal_dev, DMP_START_ADDRESS);
    if (result > worstResult)
    {
    ...
    result = loadDMPFirmware(&g_driver.internal_dev);
    if (result > worstResult)
    {
        worstResult = result;
    }
    ...
    result = writeDMPmems(&g_driver.internal_dev, ACC_SCALE, 4, &accScale[0]);
    if (result > worstResult)
    {
    ...
    result = writeDMPmems(&g_driver.internal_dev, ACC_SCALE2, 4, &accScale2[0]);
    if (result > worstResult)
    {
    ...
    result = setGyroSF(&g_driver.internal_dev, 19, 3);
    if (result > worstResult)
    {
    ...
    // polling the FIFO
    // result = intEnableDMP(true); if (result > worstResult) worstResult =
    // result;
...
ICM_20948_Status_e icm209_init(icm209_dev_t** ppt_dev)
{
    ...
    g_driver.internal_dev._gyroSF = 0; // Use this to record the GyroSF,
                                       // calculated by inv_icm20948_set_gyro_sf
    g_driver.internal_dev._gyroSFpll = 0;
    g_driver.internal_dev._enabled_Android_0 =
      0; // Keep track of which Android sensors are enabled: 0-31
    g_driver.internal_dev._enabled_Android_1 =
      0; // Keep track of which Android sensors are enabled: 32-
    g_driver.internal_dev._enabled_Android_intr_0 =
      0; // Keep track of which Android sensor interrupts are enabled: 0-31
    g_driver.internal_dev._enabled_Android_intr_1 =
      0; // Keep track of which Android sensor interrupts are enabled: 32-
